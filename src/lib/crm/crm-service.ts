...
export async function getCustomer360Profile(supabase: any, customerId: string) {
  ...
  const [quotationsRes, bookingsRes, agreementsRes, invoicesRes, paymentsRes, documentsRes, notesRes, leadsRes] = await Promise.all([
    ...
    supabase.from('invoices').select('id, invoice_number, grand_total, amount_paid, balance_due, status, issue_date').eq('customer_id', customerId).order('created_at', { ascending: false }),
    supabase.from('payments').select('id, payment_number, amount, payment_date, payment_method, status').eq('customer_id', customerId).eq('status', 'completed').order('payment_date', { ascending: false }),
